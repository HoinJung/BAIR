...
Supports both MIMIC-CXR (JSON) and IU-Chest (CSV merge) via command-line arguments.
"""
...
def main():
    ...
    parser.add_argument("--input", type=str, required=True, 
                        help="Path to the input file (JSON for MIMIC) or directory (containing CSVs for IU-Chest).")
    ...
    if args.dataset == "mimic":
        ...
        if "image" not in df.columns:
            raise ValueError("MIMIC metadata JSON must contain an 'image' field.")
        df["uid"] = df["image"].astype(str)
        
    elif args.dataset == "iu-chest":
