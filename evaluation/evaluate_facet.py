...
Modes: single-file (--input-json) or grid (--intervention-glob + --baseline-json).
"""
...
# --- Optional: rank many intervention JSONs (same metric as single-file mode) ----------
FNAME_RE_AV_AT_GS = re.compile(
    r"_intervention_av(?P<av>-?\d+(?:\.\d+)?)_at(?P<at>-?\d+(?:\.\d+)?)_gs(?P<gs>-?\d+(?:\.\d+)?)\.json$"
)
FNAME_RE_AV_ONLY = re.compile(r"_intervention_av(?P<av>[\d.]+)")


def pct_grid(n: int, d: int) -> float:
    return (100.0 * n / d) if d else 0.0


def parse_intervention_hparams(path: Path) -> Dict[str, Optional[float]]:
    m = FNAME_RE_AV_AT_GS.search(path.name)
    if m:
        return {"av": float(m.group("av")), "at": float(m.group("at")), "gs": float(m.group("gs"))}
    m2 = FNAME_RE_AV_ONLY.search(path.name)
    if m2:
        return {"av": float(m2.group("av")), "at": 1.0, "gs": 1.0}
    return {"av": None, "at": None, "gs": None}


def eval_intervention_file_subset(
    intervention_json: Path,
    baseline_data: List[Dict[str, Any]],
    max_samples: Optional[int],
    intervention_field: Optional[str],
    apply_manual_fixes: bool,
) -> Dict[str, Any]:
    with intervention_json.open("r") as f:
        data = json.load(f)
    if not isinstance(data, list):
        raise ValueError(f"Expected top-level list in {intervention_json}, got {type(data)}")
    merge_missing_baselines(data, baseline_data)
    if max_samples and max_samples > 0:
        data = data[:max_samples]

    result = evaluate_entries(
        data, intervention_field=intervention_field, apply_manual_fixes=apply_manual_fixes
    )
    stats = result["stats"]
    n = stats["n_total"]

    nr_acc = pct_grid(stats["nr_ok"], n)
    ori_acc = pct_grid(stats["ori_ok"], n)
    intv_acc = pct_grid(stats["intv_ok"], n)
    meets_requirement = (intv_acc > nr_acc) and (intv_acc > ori_acc)

    parsed = parse_intervention_hparams(intervention_json)
    return {
...
def run_intervention_grid_ranking(args: argparse.Namespace) -> None:
    ...
    if not isinstance(baseline_data, list):
        raise ValueError("Expected baseline JSON to be a list")
    ...
    intv_field = args.intervention_field.strip() if args.intervention_field else None
    max_s = getattr(args, "max_samples", 0) or None

    rows: List[Dict[str, Any]] = []
    for p in paths:
        rows.append(
            eval_intervention_file_subset(
                intervention_json=p,
                baseline_data=baseline_data,
                max_samples=max_s,
                intervention_field=intv_field,
    ...
    print("\n=== Grid / multi-file summary (FACET fairness) ===")
    print("av\tat\tgs\tn\tbaseline%\tstd_rag%\tintv%\tdelta_vs_base\tdelta_vs_std\tmeets")
    for r in rows:
        print(
    ...
    if args.output_grid_csv:
        out_csv = Path(args.output_grid_csv)
    ...
    if args.output_grid_json:
        out_json = Path(args.output_grid_json)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Single intervention / run JSON from experiments.gender_analysis (use this OR --intervention-glob).",
    )
    parser.add_argument(
        "--baseline-json",
        type=str,
        ...
        "For single-file mode: if omitted, auto-selected from --input-json name when possible.",
    )
    parser.add_argument(
        "--save-details-json",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Rank multiple intervention JSONs (grid). Requires --baseline-json.",
    )
    parser.add_argument(
        "--max-samples",
        type=int,
        default=0,
        help="Grid mode only: first N rows after merge (0 = all).",
    )
    parser.add_argument("--output-grid-csv", type=str, default=None, help="Grid mode: ranked CSV.")
    parser.add_argument("--output-grid-json", type=str, default=None, help="Grid mode: ranked JSON.")

    args = parser.parse_args()
    if args.intervention_glob:
        if not args.baseline_json:
            parser.error("--intervention-glob requires --baseline-json")
    elif not args.input_json:
        parser.error("Provide --input-json (single file) or --intervention-glob (grid)")
    return args
...
def main() -> None:
    args = parse_args()
    if args.intervention_glob:
        run_intervention_grid_ranking(args)
        return
