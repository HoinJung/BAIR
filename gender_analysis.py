...
try:
    from llm_explainer import generate_with_hf, load_llm_model, build_context
...
def load_longllmlingua(device: str):
    """Lazy-load LongLLMLingua compressor (Llama-2-7b backend), matching unified_medgemma_analysis."""
...
def apply_mspoe_position_hook_qwen(model: torch.nn.Module, scaling_factor: float, text_only: bool):
    ...
    See unified_medgemma_analysis.apply_mspoe_position_hook; adapted for 3D positions and visual span.
    """
    ...
    for idx, row in enumerate(tqdm(rows_shuffled.itertuples(), total=len(rows_shuffled), desc="Processing images")):
        ...
        try:
            ...
            if _is_qwen_vl_model(model_components):
                ...
                from llm_explainer import generate_with_hf
...
if __name__ == "__main__":
    ...
    parser.add_argument('--facet_csv', type=str, default=str(base_dir / 'data' / 'metadata' / 'facet_new_annotations.csv'))
    parser.add_argument('--image_root', type=str, default=str(base_dir / 'data' / 'raw' / 'facet' / 'image'))
    ...
    parser.add_argument('--db_path', type=str, default=str(base_dir / 'data' / 'metadata' / 'profession_database_fixed.json'))
    parser.add_argument('--exclude_path', type=str, default=str(base_dir / 'data' / 'metadata' / 'excluding_list.json'))
    ...
    # FACET + Qwen2.5-VL: Ms-PoE, LongLLMLingua, BAIR combinations (see unified_medgemma_analysis.py).
