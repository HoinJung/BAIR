...
evaluate_intervention_gender_fairness: empty/whitespace, strip length < MIN_GEN_STRLEN,
...
CACHE_FILE = BASE_DIR / "srr_bert_f1_eval_cache.json"
...
BASE_DIR = Path(__file__).resolve().parent
DATASET_PATHS = {
    "mimic": str(BASE_DIR / "data" / "generated" / "mimic_reports_with_pseudo_labels_dual.csv"),
    "iu-chest": str(BASE_DIR / "data" / "generated" / "indiana_reports_with_pseudo_labels_dual.csv"),
}
...
# --- Minimal generation failure (keep in sync with evaluate_intervention_gender_fairness.py) ---
MIN_GEN_STRLEN = 5
...
def compute_metrics(
    ...
    if texts_by_cond is not None:
        print(
            ...
            f"evaluate_intervention_gender_fairness: empty/whitespace, strip length < {MIN_GEN_STRLEN}, "
...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=["mimic", "iu-chest"], required=True)
