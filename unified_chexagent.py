#!/usr/bin/env python3
"""
Unified CheXagent Analysis for IU-Chest and MIMIC-CXR datasets.
Includes Baselines, BAIR, Ms-PoE, Positional RAG, and LongLLMLingua interventions.
Features Dynamic Visual Token Calculation, Sharpness Boosting, and Tail Token Protection.
"""
...
MODEL, PROCESSOR, GENERATION_CONFIG = None, None, None
...
def get_unified_dataset(args) -> List[Dict]:
    ...
    if args.dataset == "iuchest":
        ...
        db_path = base_dir / "data" / "generated" / "mimic_nih_retrieval_dataset.json"
...
def load_chexagent_intervention_model(device: str):
    ...
    if MODEL is None:
        ...
        # =================================================================
        # [ULTIMATE FIX] Registry & Class Override (Version-Safe)
        # =================================================================
        ...
        PROCESSOR = AutoProcessor.from_pretrained(CHEXAGENT_ID, trust_remote_code=True)
        GENERATION_CONFIG = GenerationConfig.from_pretrained(CHEXAGENT_ID)
        ...
        if hasattr(MODEL, "language_model") and hasattr(MODEL.language_model.model, "layers"):
            layer_type = type(MODEL.language_model.model.layers[0].self_attn).__name__
...
def format_chex_prompt(question: str, context: Optional[str] = None, instruction: Optional[str] = None) -> str:
    ...
    if instruction: user_prompt = f"{instruction}\n\n{user_prompt}"
...
def apply_mspoe_position_hook(model, scaling_factor: float, text_only: bool, num_visual_tokens: int):
...
def generate_standard_chexagent(image_path: str, question: str, context: Optional[str], max_new_tokens: int, instruction: Optional[str], device: str) -> str:
    ...
    inputs = {k: v.to(device).to(torch.bfloat16) if k == 'pixel_values' else v.to(device) for k, v in inputs.items()}
    ...
    if "ASSISTANT: <s>" in res: return res.split("ASSISTANT: <s>", 1)[1].split("</s>")[0].split("USER:")[0].strip()
    return res.strip()
...
def generate_with_bair_and_mspoe_chexagent(
    ...
    clean_inputs = {k: v.to(device).to(torch.bfloat16) if k == 'pixel_values' else v.to(device) for k, v in clean_inputs.items()}
    ...
    # --- Calibration Pass ---
    set_bottleneck_intervention(True, num_visual_tokens=num_visual_tokens, calibration_run=True, reset_layer=True, alpha_v=alpha_v, alpha_t=alpha_t, gamma_s=gamma_s, question_tokens=safe_tail_tokens, use_madrag=use_madrag)
    with torch.no_grad():
        model(**clean_inputs)
    del clean_inputs
    ...
    gen_inputs = {k: v.to(device).to(torch.bfloat16) if k == 'pixel_values' else v.to(device) for k, v in gen_inputs.items()}

    set_bottleneck_intervention(True, num_visual_tokens=num_visual_tokens, calibration_run=False, reset_layer=True, alpha_v=alpha_v, alpha_t=alpha_t, gamma_s=gamma_s, question_tokens=safe_tail_tokens, use_madrag=use_madrag)
...
def main():
    ...
    parser.add_argument('--uids', type=str, default=None, help="Comma-separated UIDs to process (e.g. '2353,182,857'). Overrides test_fraction/num_samples.")
    ...
    parser.add_argument('--output_dir', type=str, default='generation_results_chexagent')
    ...
    if (args.use_longllmlingua or args.use_longllmlingua_combo) and PromptCompressor is None:
        msg = (
            "LongLLMLingua requires the `llmlingua` package. Install with: pip install llmlingua\n"
        )
    ...
    mode_parts = []
    if args.generate_baselines: mode_parts.append("baselines")
    ...
    if args.use_mspoe: mode_parts.append(f"{'mspoe_text' if args.combo_text_only else 'mspoe_full'}_{args.gt_position}")
    if args.use_madrag: mode_parts.append(f"madrag_{args.gt_position}")
    if args.use_combo: mode_parts.append(f"combo_{'text' if args.combo_text_only else 'full'}_av{args.alpha_v}_{args.gt_position}")
    if args.use_madrag_combo: mode_parts.append(f"madrag_combo_av{args.alpha_v}_{args.gt_position}")
    if args.use_longllmlingua: mode_parts.append(f"longllmlingua_{args.gt_position}")
    if args.use_longllmlingua_combo: mode_parts.append(f"lll_combo_av{args.alpha_v}_{args.gt_position}")
    if args.context_mode == "gt_only": mode_parts.append("ctx_gt_only")
    ...
    out_file = Path(args.output_dir) / f"{args.dataset}_chexagent_results{suffix}.json"
    ...
    def _has_valid_value(entry, key):
        ...
        if isinstance(v, str) and (not v.strip() or "[Error]" in v or "Cons Cons" in v):
            return False
        return True
    ...
    for entry in tqdm(all_results, desc=f"Processing {args.dataset} (CheXagent)"):
        ...
        try:
            if args.generate_baselines:
                if not _has_valid_value(entry, "no_retrieval_answer"):
                    entry["no_retrieval_answer"] = generate_standard_chexagent(image_path, q, None, args.max_new_tokens, ins, device)
                if not _has_valid_value(entry, "oracle_answer"):
                    entry["oracle_answer"] = generate_standard_chexagent(image_path, q, ctx_mid, args.max_new_tokens, ins, device)
                if not _has_valid_value(entry, "prompt_baseline_answer"):
                    entry["prompt_baseline_answer"] = generate_standard_chexagent(image_path, q, ctx_mid, args.max_new_tokens, s_ins, device)
            ...
            if args.use_intervention:
                ...
                if not _has_valid_value(entry, key):
                    ...
                    entry[key] = generate_with_bair_and_mspoe_chexagent(
            ...
            if args.use_mspoe:
                ...
                if not _has_valid_value(entry, key):
                    entry[key] = generate_with_bair_and_mspoe_chexagent(image_path, q, target_ctx, args.max_new_tokens, ins, 0.0, 0.0, 1.0, device, 1.5, args.combo_text_only, use_madrag=False)

            if args.use_madrag:
                key = "madrag_answer" if args.gt_position == 'mid' else f"madrag_{args.gt_position}_answer"
                if not _has_valid_value(entry, key):
                    entry[key] = generate_with_bair_and_mspoe_chexagent(image_path, q, target_ctx, args.max_new_tokens, ins, 0.0, 0.0, 1.0, device, 1.0, False, use_madrag=True)
                
            if args.use_combo:
                key = f"combo_{args.gt_position}_answer"
                if not _has_valid_value(entry, key):
                    entry[key] = generate_with_bair_and_mspoe_chexagent(image_path, q, target_ctx, args.max_new_tokens, ins, args.alpha_v, args.alpha_t, args.gamma_s, device, 1.5, args.combo_text_only, use_madrag=False)

            if args.use_madrag_combo:
                key = f"madrag_combo_{args.gt_position}_answer"
                if not _has_valid_value(entry, key):
                    entry[key] = generate_with_bair_and_mspoe_chexagent(image_path, q, target_ctx, args.max_new_tokens, ins, args.alpha_v, args.alpha_t, args.gamma_s, device, 1.0, False, use_madrag=True)
            ...
            if args.use_longllmlingua:
                ...
                if not _has_valid_value(entry, key):
                    ...
                    entry[key] = generate_standard_chexagent(image_path, q, comp_ctx, args.max_new_tokens, ins, device)
                
            if args.use_longllmlingua_combo:
                key = f"longllmlingua_combo_{args.gt_position}_answer"
                if not _has_valid_value(entry, key):
                    comp_ctx = compress_with_longllmlingua(target_ctx, q, ins, comp_device, args.compression_ratio)
                    entry[key] = generate_with_bair_and_mspoe_chexagent(image_path, q, comp_ctx, args.max_new_tokens, ins, args.alpha_v, args.alpha_t, args.gamma_s, device, 1.0, False)
                
        except Exception as e:
            print(f"\n[Error] Failed on {entry['uid']}: {e}")
            traceback.print_exc()
            print("\nStopping execution to allow for debugging...")
            break
